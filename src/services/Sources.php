...
class Sources extends Component
{
    ...
    public function getSourceOverrides(string $handle): array
    {
        if ($this->_overrides === null) {
            $this->_overrides = Craft::$app->getConfig()->getConfigFromFile('video-picker');
        }
    ...
    private function _getSourceRecordById(int $sourceId = null): ?SourceRecord
    {
        if ($sourceId !== null) {
            ...
            if (!$sourceRecord) {
                throw new Exception(Craft::t('video-picker', 'No source exists with the ID “{id}”.', ['id' => $sourceId]));
            }
        } else {
