...
After switching the type of your fields, you'll then need to create new Sources for your video providers. Navigate to Video Picker > Sources in the Craft control panel to set up the same OAuth sources you had in Dukt Videos (typically YouTube and/or Vimeo), plus any credential sources you want (Dailymotion, Mux, Wistia). Unfortunately, the two plugins' OAuth handling is incompatible and cannot be migrated automatically, so re-authorize OAuth sources manually. Credential sources use API tokens from the provider dashboard.
